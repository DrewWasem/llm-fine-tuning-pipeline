...
- [ ] Create `requirements.txt` with pinned core deps (torch, transformers, peft, trl, accelerate, datasets, spacy)
- [ ] Create `pyproject.toml` or `setup.py` for package installation
- [ ] Set up directory structure with `__init__.py` files
- [ ] Create `src/config/settings.py` with Pydantic-based config loading
- [ ] Create `configs/domains/legal.yaml` (first domain config)
- [ ] Create `configs/training/lora_adapt.yaml` (simplest training config)
- [ ] Add basic `Dockerfile` for reproducible environment
- [ ] Add `Makefile` with common commands (`make install`, `make test`, `make lint`)
...
### 1a — Data Loader
- [ ] Implement `src/data/loaders/base_loader.py` — abstract interface for all loaders
- [ ] Implement `src/data/loaders/legal_loader.py` — load legal docs from local files / HuggingFace datasets (e.g., `pile-of-law`)
- [ ] Add CLI entry point: `python scripts/build_corpus.py --domain legal --source pile-of-law`

### 1b — Quality Filtering
- [ ] Implement `src/corpus/quality/filters.py` — length filter, language filter, quality score
- [ ] Implement `src/corpus/quality/deduplication.py` — MinHash-based near-duplicate removal

### 1c — Terminology Extraction
- [ ] Implement `src/corpus/terminology/extractor.py` — TF-IDF + spaCy NER hybrid extraction
- [ ] Add CLI: `python scripts/extract_terminology.py --corpus data/processed/legal_corpus`
- [ ] Output: `data/terminology/legal_terms.json`

### 1d — Data Formatting
- [ ] Implement `src/data/formatters/domain_chat.py` — convert raw docs to training format (chat template)
- [ ] Implement `src/data/processors/tokenizer.py` — tokenization with HF tokenizer, packing support
...
### 2a — Model Loading
- [ ] Implement `src/models/loader.py` — load base model + tokenizer with quantization support (4-bit via bitsandbytes)
- [ ] Create `src/models/domain_configs/legal_llama.yaml` — model-specific params

### 2b — LoRA Training
- [ ] Implement `src/adaptation/strategies/lora_adapt.py` — LoRA fine-tuning using `peft` + `trl`
- [ ] Implement `src/adaptation/stages/instruction_tune.py` — SFT training loop with `SFTTrainer`
- [ ] Add training config: `configs/training/instruction_tune.yaml`
- [ ] Integrate W&B logging for loss curves, learning rate, GPU utilization

### 2c — Data Mixing
- [ ] Implement `src/adaptation/curriculum/data_mixer.py` — mix domain data with general data (configurable ratio) to prevent catastrophic forgetting

### 2d — Training Script
- [ ] Implement `scripts/train_domain_model.py` — CLI orchestrator
...
### 3a — Domain Benchmark
- [ ] Implement `src/evaluation/benchmarks/legal/bar_exam.py` — multiple-choice legal QA (can use MMLU professional_law subset)
- [ ] Implement `src/evaluation/benchmarks/legal/contract_analysis.py` — contract clause extraction/classification

### 3b — Knowledge Retention
- [ ] Implement `src/evaluation/retention/general_knowledge.py` — MMLU subset to check general knowledge retention
- [ ] Implement `src/evaluation/retention/catastrophic_forgetting.py` — compare base vs adapted model on general benchmarks

### 3c — Terminology Accuracy
- [ ] Implement `src/evaluation/terminology/term_accuracy.py` — test if model uses domain terms correctly in generated text

### 3d — Evaluation Script
- [ ] Implement `scripts/evaluate_domain.py` — CLI orchestrator
  - Loads model → runs benchmarks → outputs JSON report
- [ ] Output a comparison: base model vs adapted model, per-benchmark scores
...
### 5a — Integration
- [ ] Create `scripts/run_pipeline.py` — orchestrates corpus → train → evaluate in sequence
- [ ] Add `configs/domains/legal.yaml` as a complete pipeline config (all stages)

### 5b — Tests
- [ ] Unit tests for corpus building (filters, dedup, terminology)
- [ ] Unit tests for data formatting and tokenization
- [ ] Unit tests for evaluation metric calculations
- [ ] Integration test: mini pipeline with a tiny model (e.g., `TinyLlama`) on a small dataset
- [ ] Add CI workflow (GitHub Actions) — lint + unit tests on every PR

### 5c — Documentation
- [ ] Write `README.md` — quick start, installation, usage examples
- [ ] Add example output / sample evaluation report
